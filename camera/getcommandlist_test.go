...
var _ = Describe("GetCommandList", func() {
	When("called", func() {
		It("should return the list of cgi methods", func() {
			client, mux, teardown := testSetup()
			defer teardown()

			mux.HandleFunc("/get_commandlist.cgi", func(w http.ResponseWriter, r *http.Request) {
				Expect(r.Method).To(Equal("GET"))
				fmt.Fprint(w, getCommandListResponse)
			})

			cmdList, err := client.GetCommandList()
			Expect(err).NotTo(HaveOccurred())
			Expect(cmdList.Cgi).To(HaveLen(23))
		})
	})
})

const getCommandListResponse = `
<?xml version="1.0"?>
<oishare>
